...
public class Decryptor implements IComand {
    ...
    public Decryptor(Settings settings, KeyCreator key, ImageViewData image) {
    ...
    public void execute() throws IOException {
        ...
        switch (settings.getAlgorythm()) {
            case XOR:
                result = XOR.decrypt(value, key, f);
                break;
            case RC4:

                try {
                    result = RC4.decrypt(value, key);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                break;
